...
尽管 Redis Cluster 在分区时倾向于 CP，但在正常运行和数据同步方面，它是一个最终一致性系统：

异步复制 (Asynchronous Replication)： Redis 主从节点之间采用异步复制。这意味着主节点接收到一个写入请求并返回成功后，数据可能尚未同步到所有从节点。

在从节点上进行读取（如果允许），可能会读到旧数据，这是 AP 系统的特征。

写入丢失窗口： 在主节点发生故障并自动进行故障转移（Failover）时，如果最新的写入请求尚未异步复制到任何从节点，那么这个已向客户端确认的写入可能会丢失。这也表明它不具备严格的 C。
