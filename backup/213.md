...
为了确保数据的可靠性和系统的持续可用性，Kafka的分区通过复制（Replication）机制实现了高可用性。Kafka主题可以设置一个大于1的复制因子，这个因子决定了有多少个Broker会存储主题数据的副本。被复制的分区会被分配给其他Broker，这些Broker被称为该分区的Follower。
